...
/**
 * <b>GameJoltAPI</b><br/>
 * This is the main class that you use to implement the GameJolt Trophy,
 * DataStore and HighScore systems.
 *
 * @since 0.90
 * @version 0.99
...
public class GameJoltAPI {
    ...
    /**
     * Whether or not this object should print debug messages.
     */
    private boolean verbose = false;
    /**
     * Whether or not this object has been verified using verifyUser().
     */
     ...
     *
     * This method will try to read the Game Jolt Quick Play file
     * (gjapi-credentials.txt). If that file exists, then it will attempt to
     * verify the user.
     *
     ...
     *
     * @param b whether the API should print out debug informationto the
     * Console.
     */
    ...
    /**
     * reloads the quickplay-file. This will reset the verified-status. This
     * should normally have no effect, since the quickplay-file is created when
     * the user starts a quickplay-game, and not while the game is running.
     */
    ...
    /**
     * Gets the user object of the quickplay user if the game has the
     * gjapi-credentials.txt file. if you only want to get the name and token,
     * use {@link #getQuickplayUserCredientals()}
     *
    ...
    /**
     * gets the User object of the user with a certain name This User will not
     * have a token
     *
     * @param name the name of the user
     * @return the Userobject of the user or null if no user with this name
     * exists
     */
    ...
    /**
     * gets the User object of the user with a certain id This User will not
     * have a token
     *
    ...
    public User getVerifiedUser() {
        if (this.verified) {
            ...
            if (this.verbose) {
                System.err.println("GameJoltAPI: Could not get the (currently verified) user.");
            }
            return null;
        }
    }
    ...
    /**
     * Retrieve a list of Highscores from GameJolt for either a game or the
     * verified user.
     *
     * @param onlyCurrentUser If set to true, this will retrieve highscores of 
     * only the verified user. Otherwise, returns highscores from everybody.
     * @return An array of Highscore objects on success, an empty array or null
     * on failure.
     */
    public ArrayList<Highscore> getHighscores(boolean onlyCurrentUser) {
        return this.getHighscores(onlyCurrentUser, 100);
    }

    /**
     * Retrieve all of the Highscores of the Highscoretable from GameJolt for
     * the game in an array.
     *
     ...
     * on failure.
     */
    public ArrayList<Highscore> getHighscores(int id, boolean onlyCurrentUser) {
        return this.getHighscores(id, onlyCurrentUser, 100);
    }
     ...
     * on failure.
     */
    public ArrayList<Highscore> getHighscores(boolean onlyCurrentUser, int limit) {
        return getHighscores(0, onlyCurrentUser, limit);
    }
     ...
     * on failure.
     */
    public ArrayList<Highscore> getHighscores(int id, boolean onlyCurrentUser, int limit) {
        if (onlyCurrentUser && !this.verified) {
            if (verbose) {
                System.err.println("GameJoltAPI: Could not get the Highscores for the verified user as the user is not verified.");
            }
            return null;
        }
        ...
        try {
            HashMap<String, String> params = new HashMap<String, String>();
            if (!onlyCurrentUser) { // all highscores
                if (id != 0) {
     ...
     *
     * @param score The String of the score, e.g. "5 Grapefruits". This is shown
     * on the site.
     * @param sort The sortable value of the score, e.g. 5. This is shown on the
     * site.
     ...
     * @param sort The sortable value of the score, e.g. 5. This is shown on the
     * site.
     * @param extra Extra information to be stored about this score as a String,
     * such as how the score was made, or time taken. This is not shown on the
     * site.
     * @return true if successful, otherwise false.
     */
    ...
    public boolean addHighscore(int id, String score, int sort, String extra) {
        if (!this.verified) {
            if (verbose) {
                System.err.println("GameJoltAPI: Could not add the High Score because the user is not verified.");
            }
            return false;
        }
        String response = null;
        try {
            ...
            if (response.contains("success:\"false\"") || response.equals("REQUEST_FAILED")) {
                if (verbose) {
                    System.err.println("GameJoltAPI: Could not add the High Score.");
                }
                if (verbose) {
                    System.out.println(response);
                }
     ...
     *
     * @param guest_username The desired name of the guest you want to add the
     * highscore for.
     * @param score The String of the score, e.g. "5 Grapefruits". This is shown
     * on the site.
     * @param sort The sortable value of the score, e.g. 5. This is shown on the
    ...
    public boolean addHighscore(int id, String guest_username, String score, int sort, String extra) {
        ...
        try {
            ...
            if (response.contains("success:\"false\"") || response.equals("REQUEST_FAILED")) {
                if (verbose) {
                    System.err.println("GameJoltAPI: Could not add the Guest High Score.");
                }
     ...
     */
    /**
     * updates the data of an existing entry on the gamejolts servers by
     * performing a {@link DataStoreOperation} between the data on the Server
     * and the values
     *
     * @param type the Type of the Data Store. Should be either
     * DataTypeStore.USER or DataTypeStore.GAME.
     * @param key key for which to store the data. You use this key to
     * retrieve the DataStore.
     * @param operation the operation to perform on the entry
     * @param value The value to work with.
     * @return The newly updated Data Store
     */
...
//				response = this.request("data-store/update/", "key=" + key + "&operation=" + operation.toString().toLowerCase() + "&value=" + value);
        ...
        if (!response.substring(0, 7).equals("SUCCESS")) {
            if (verbose) {
                System.err.println("could not update DataStore");
            }
            return null;
        }
        String data = response.substring(9);
        return new DataStore(key, data, type);
    }

    /**
     * Adds a piece of data to Game Jolt's servers replacing any that already
     * exists with the key and type given.
     *
     * @param type The type of the Data Store. Should be either
     * DataTypeStore.USER or DataTypeStore.GAME.
     * @param key The key for which to store the data. You use this key to
     * retrieve the DataStore.
     * @param data Data to keep on Game Jolt servers. This data is normally xml,
     * json, or serialised memory.
    ...
    public DataStore setDataStore(DataStoreType type, String key, String data) {
        ...
        try {
            ...
            if (response.contains("success:\"false\"")) {
                if (verbose) {
                    System.err.println("GameJoltAPI: Could not add " + type + " DataStore with Key \"" + key + "\".");
                }
    ...
    /**
     * Remove a piece of data from Game Jolt's servers as specified by the type
     * and key.
     *
     * @param type The type of the Data Store. Should be either
     * DataTypeStore.USER or DataTypeStore.GAME.
     * @param key The key for which to remove the data.
     * @return True if the DataStore was removed successfully. Note this will
     * return false if no item exists with the key given.
     */
    public boolean removeDataStore(DataStoreType type, String key) {
        ...
        if (response.contains("success:\"false\"")
            ...
            if (verbose) {
                System.err.println("GameJoltAPI: Could not get " + type + " DataStore with Key \"" + key + "\".");
            }
            if (verbose) {
                System.out.println(response);
            }
    ...
    /**
     * Retrieve a list of DataStore Objects for the type specified by the
     * parameter. This method loops through all of the keys and creates all
     * objects using a new http request for each key. For this reason, it is
     * slow, and it is advised that you use GameJoltAPI.getDataStoreKeys(); In
     * Big O notation, this method takes O(n + 1) time.
     *
     * @param type The Type of keys to get, either DataStoreType.USER or
     * DataStoreType.GAME.
     * @return a list of Data Store keys for the type specified by the
     * parameters.
     */
    ...
    /**
     * Retrieve a list of Data Store keys for the type specified by the
     * parameter.
     *
     * @param type The Type of keys to get, either DataStoreType.USER or
     * DataStoreType.GAME.
        ...
        try {
            if (type == DataStoreType.GAME) {
                ...
                if (!keys[0].trim().equals("success:\"true\"")) {
                    if (verbose) {
                        System.err.println("GameJoltAPI: Could not get " + type + " DataStores.");
                    }
                    return null;
                }
    ...
    /**
     * Retrieve a piece of data from Game Jolt's servers as specified by type
     * and key.
     *
     * @param type The type of the Data Store. Should be either
     * DataTypeStore.USER or DataTypeStore.GAME.
    ...
    public DataStore getDataStore(DataStoreType type, String key) {
        ...
        if (!response.substring(0, 7).equals("SUCCESS")) {
            if (verbose) {
                System.err.println("GameJoltAPI: " + response.substring(9));
            }
            if (verbose) {
                System.out.println(response);
            }
     ...
     */
    /**
     * Open a new play session with Game Jolt. This requires a user to be
     * verified (logged in). You should do this before you ping or update. This
     * will close the previously open session.
     *
     * @return true if a new play session was opened successfully. false if
     * request fails or if the user is not verified.
     */
     ...
     *
     * @return true if the session exists, false if no session exists, the
     * player is not verified or an Exception occured
     */
    ...
    /**
     * Update the current play session with Game Jolt. You should call this
     * every 60 seconds as Game Jolt closes play sessions after 120 seconds of
     * inactivity. This method will set the user as ACTIVE. Use the other method
     * to set the user as IDLE. This requires a user to be verified (logged in).
     * This method will return false if there is no session to update.
     *
     * @return true if the play session was updated (pinged) successfully, false
     * if request fails (no current session) or if the user is not verified.
     */
     ...
     * Update the current play session with Game Jolt. You should call this
     * every 60 seconds as Game Jolt closes play sessions after 120 seconds of
     * inactivity. This requires a user to be verified (logged in). This method
     * will return false if there is no session to update.
     *
     * @param active You can set the game player as ACTIVE with true or IDLE
     * with false. A good example of this would be sending an idle message if
     * the user is on a menu or pause screen.
     * @return true if the play session was updated (pinged) successfully, false
     * if request fails (no current session) or if the user is not verified.
     */
    ...
    /**
     * Close the current play session at Game Jolt. This requires a user to be
     * verified (logged in). You should do this when closing or exiting your
     * game. This method will return false if there is no session to close.
     *
     * @return true if the play session was closed successfully, false if
     * request fails (no current session) or if the user is not verified.
     */
    ...
    /**
     * Get a list of trophies filtered with the Achieved parameter. The
     * parameter can be Achieved.TRUE for achieved trophies, Achieved.FALSE for
    ...
    public Trophy getTrophy(int trophyId) {
        ...
        if (response.contains("success:\"false\"")) {
            if (verbose) {
                System.err.println("GameJoltAPI: Could not get Trophy with Id " + trophyId + ".");
            }
            return null;
        }
        if (response.contains("REQUIRES_AUTHENTICATION")) {
            if (verbose) {
                System.err.println("GameJoltAPI: User Authentication is required for this action.");
            }
            return null;
        }
        if (verbose) {
    ...
    /**
     * Get the server time.
     * @return a ServerTime object if the request succeeds, or null if it fails.
     */
    ...
    public boolean verifyUser(String username, String userToken) {
        ...
        for (String line : lines) {
            ...
            try {
                ...
                if (verbose) {
                    System.err.println("GameJoltAPI: Could not verify user because the response from GJ was invalid.");
                }
                return false;
            }
        }
     ...
     * without the "game-api/" part.
     * @param params A map of the parameters you want to include. Note that if
     * the user is verified you do not have to include the
     * username/user_token/game_id.
    ...
    /**
     * Perform a GameJolt API request. Use this one if you know your HTTP
     * requests.
     *
     * @param method The API method to call. Note that gamejolt.com/api/game/ is
     * already prepended.
     * @param paramsLine The GET request params, such as
     * "trophy_id=23&achieved=empty".
     ...
     * Make a request to the GameJolt API.
     *
     * @param method The GameJolt API method, such as "add-trophy", without the
     * "game-api/" part.
     * @param paramsLine The GET request params, such as
     * "trophy_id=23&achieved=empty".
     * @param requireVerified This is only set to false when checking if the
     * user is verified.
     * @return
     */
     ...
     * @param requireVerified This is only set to false when checking if the
     * user is verified.
     * @return The HTTP response to the request, or "REQUEST_FAILED" if the 
     * request fails or if an error occurs.
     */
    private String request(String method, HashMap<String, String> params, boolean requireVerified) {
        try {
            ...
            // the constructed URL to send as a request
            String urlString = this.getRequestURL(method, params);
            if (verbose) {
    ...
    public String openURLAndGetResponse(String urlString) {
        try {
            ...
            //e.printStackTrace();
            if (this.verbose) {
                System.err.println("GameJoltAPI: " + e.getMessage());
            }
            return "REQUEST_FAILED";
        }
    }
